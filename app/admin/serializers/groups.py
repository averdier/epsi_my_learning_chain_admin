...
from .nested import student_nested, claim_nested, campus_nested
...
group_minimal_model = api.model('Group minimal model', {
    ...
    'campus_id': fields.String(required=True, description='Campus ID', attribute=lambda p: p.project.campus.id),
...
group_full_model = api.inherit('Group full model', group_model, {
    ...
    'campus': fields.Nested(campus_nested, required=True, description='Campuss', attribute=lambda p: p.project.campus)
})
